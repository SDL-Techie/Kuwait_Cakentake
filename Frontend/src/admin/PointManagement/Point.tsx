...
import { getPointSetting, savePointSetting, PointSetting } from '../../services/pointsettingService';
import './Point.css';

const emptySettings: PointSetting = {
    min_purchase: 0,
    points_earned: 0,
    points_needed: 0,
    reward_percentage: 0,
...
const Point = () => {
    const [loading, setLoading] = useState(true);
    const [isFormOpen, setIsFormOpen] = useState(false);
    const [settings, setSettings] = useState<PointSetting>(emptySettings);
    const [toast, setToast] = useState({ show: false, message: '', type: '' });
    ...
    const fetchSettings = async () => {
        try {
            setLoading(true);
            const data = await getPointSetting();
    ...
    const handleSave = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await savePointSetting(settings);
            showToast("Loyalty Rules Updated Successfully!", "success");
            setIsFormOpen(false);
            fetchSettings();
        } catch (err) {
    ...
    return (
        <div className="rasi-users-container">
            ...
            {isFormOpen && (
                <div className="form-card-wrapper animate-slide-down">
                    <div className="form-card-header">
                        <h2>{settings.min_purchase ? "Modify Loyalty Rules" : "Create Loyalty Rules"}</h2>
                        <button className="close-form-icon-btn" onClick={() => setIsFormOpen(false)}>
                            <FaTimes />
                        </button>
                    </div>
                        ...
                        <div className="settings-grid">
                            <div className="input-group">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setSettings({ ...settings, min_purchase: Number(e.target.value) })}
                                    required
                                    min="0"
                                />
                            </div>
                            <div className="input-group">
                                ...
                                <input
                                    type="number"
                                    value={settings.points_earned}
                                    onChange={(e) => setSettings({ ...settings, points_earned: Number(e.target.value) })}
                                    required
                                    min="0"
                                />
                            </div>
                            <div className="input-group">
                                ...
                                <input
                                    type="number"
                                    value={settings.points_needed}
                                    onChange={(e) => setSettings({ ...settings, points_needed: Number(e.target.value) })}
                                    required
                                    min="0"
                                />
                            </div>
                            ...
                            <div className="input-group">
                                ...
                                <input
                                    type="number"
                                    value={settings.coupon_validity_days || 0}
                                    onChange={(e) => setSettings({ ...settings, coupon_validity_days: Number(e.target.value) })}
                                    required
                                    min="1"
                                />
                            </div>
            ...
            <div className="dashboard-cards-section-wrapper">
                ...
                {loading ? (
                    ...
                    <div className="metrics-display-grid">
                        <div className="metric-card purchase-rule-card">
                            <div className="metric-card-top">
                                <div className="metric-icon-box box-orange"><FaShoppingBag /></div>
                                <span className="status-indicator active">Active</span>
                            </div>
                            <div className="metric-card-body">
                                <p className="metric-label">Purchase Threshold</p>
                                <h2 className="metric-value">₹{settings.min_purchase}</h2>
                                <p className="metric-description">
                                    Amount Required For {settings.points_earned} Points.
                                </p>
                            </div>
                            <div className="metric-card-footer">
                                <span className="footer-highlight">Yields +{settings.points_earned} Pts</span>
                            </div>
                        </div>

                        <div className="metric-card threshold-rule-card">
                            <div className="metric-card-top">
                                <div className="metric-icon-box box-gold"><FaCoins /></div>
                                <span className="status-indicator active">Active</span>
                            </div>
                            <div className="metric-card-body">
                                <p className="metric-label">Point Target</p>
                                <h2 className="metric-value">{settings.points_needed} <span className="unit-text">Pts</span></h2>
                                <p className="metric-description">
                                    If you reach {settings.points_needed} points, you get {settings.reward_percentage}% off
                                </p>
                            </div>
                            <div className="metric-card-footer">
                                <span className="footer-highlight text-gold">Milestone Limit</span>
                        ...
                        <div className="metric-card benefit-rule-card">
                            <div className="metric-card-top">
                                <div className="metric-icon-box box-brown"><FaPercent /></div>
                                <span className="status-indicator active">Active</span>
                            </div>
                            <div className="metric-card-body">
                                <p className="metric-label">Claimable Reward</p>
                                <h2 className="metric-value">{settings.reward_percentage}% <span className="unit-text">OFF</span></h2>
                                <p className="metric-description">
                                    Discount scale applied to the billing order once voucher is redeemed.
                                </p>
                            </div>
                        ...
                        <div className="metric-card validity-rule-card">
                            <div className="metric-card-top">
                                <div className="metric-icon-box box-blue"><FaCalendarAlt /></div>
                                <span className="status-indicator active">Active</span>
                            </div>
                            <div className="metric-card-body">
                                <p className="metric-label">Coupon Lifespan</p>
                                <h2 className="metric-value">{settings.coupon_validity_days || 7} <span className="unit-text">Days</span></h2>
                                <p className="metric-description">
                                    Validity frame before the generated voucher becomes inactive.
                                </p>
                            </div>
