...
/* ============================================================
   IMAGE UPLOAD FIELD — drag & drop, preview, replace, remove
   (used by Categories, Subcategories, Products, Combos, and Addons)
============================================================ */
const ImageUploadField: React.FC<{
  label: string; preview: string; fileId: string;
  onFileChange: (file: File, url: string) => void;
...
// ── CHANGED: Product now carries subcategoryId / subcategoryName ──
interface Product     { id: string; name: string; description: string; categoryId: string; categoryName?: string; subcategoryId: string; subcategoryName?: string; price: number; originalPrice?: number; stock: number; unit: string; image: string; ingredients: string; isAvailable: boolean; }
interface Category    { id: string; name: string; description?: string; image?: string; isActive?: boolean; }
interface SubCategory { id: string; name: string; parentCategoryId: string; parentCategoryName?: string; description?: string; image?: string; isActive: boolean; }
interface Variant     { id: string; name: string; priceModifier: number; isActive: boolean; productId?: string; productName?: string; }
interface Flavor      { id: string; name: string; priceModifier: number; isActive: boolean; variantId?: string; variantName?: string; }
...
export const MenuManagement: React.FC = () => {
  ...
  const [activeCatFilter,  setActiveCatFilter]  = useState('all');
  // ── NEW: selected subcategory chip in the Products tab ──
  ...
  const [pOrigPrice, setPOrigPrice] = useState(''); const [pDesc, setPDesc] = useState('');
  const [pCatId, setPCatId] = useState(''); const [pImage, setPImage] = useState('');
  // ── NEW: subcategory chosen in the product form ──
  ...
  // ── CHANGED: takes a subcategory id too, and maps subcategory fields ──
  const fetchProducts = useCallback(async (catId?: string, subId?: string) => {
    setLoading(true);
    try {
      // The service already ignores undefined / 'all'
      const res = await menuManagementService.getProducts(catId, subId);
      const data: any[] = Array.isArray(res.data) ? res.data : (res.data.products || []);
      setProducts(data.map((item: any) => ({
        id:              String(item.id),
        name:            item.name || '',
        description:     item.description || '',
        categoryId:      String(item.category?.id || item.category_id || ''),
        categoryName:    item.category?.name || item.category_name || '',
        subcategoryId:   String(item.subcategory?.id || item.subcategory_id || ''),
  ...
  // ── CHANGED: `silent` = true skips the loading spinner (used on first page load) ──
  const fetchSubCats = useCallback(async (silent: boolean = false) => {
  ...
  useEffect(() => {
    ...
    fetchCategories();
    fetchAllVariants();
    fetchSubCats(true);   // ── NEW: load subcategories up front so the chips can use them ──
  }, []); // eslint-disable-line

  useEffect(() => {
    setSearchTerm('');
    if (activeTab === 'products')      fetchProducts(activeCatFilter, activeSubFilter);
    if (activeTab === 'categories')    fetchCategories();
    if (activeTab === 'subcategories') fetchSubCats();
    if (activeTab === 'variants')      fetchVariants(variantProductId || undefined);
    if (activeTab === 'addons')        fetchAddons();
  ...
  /* ============================================================
     PRODUCT FILTER HANDLERS  (NEW)
  ============================================================ */
  // Clicking a category: reset the subcategory chip and reload
  const handleCategoryFilter = (catId: string) => {
    setActiveCatFilter(catId);
    setActiveSubFilter('all');
    fetchProducts(catId, 'all');
  };

  // Clicking a subcategory: keep the category, narrow by subcategory
  const handleSubFilter = (subId: string) => {
    setActiveSubFilter(subId);
    fetchProducts(activeCatFilter, subId);
  };

  // Subcategories that belong to the currently selected category
  const visibleSubCats = subCats.filter(sc => sc.parentCategoryId === activeCatFilter);
  ...
  const openAddProduct = () => {
    setSelProduct(null);
    setPName(''); setPPrice('0'); setPOrigPrice(''); setPDesc('');
    // Pre-select the category/subcategory currently being filtered, if any
    setPCatId(activeCatFilter !== 'all' ? activeCatFilter : (categories[0]?.id || ''));
    setPSubId(activeCatFilter !== 'all' && activeSubFilter !== 'all' ? activeSubFilter : '');
    setPImage(''); setPFile(null);
    setPAvailable(true); setPStock('0'); setPUnit('piece'); setPIngredients('');
    setProductModal(true);
  };
  const openEditProduct = (p: Product) => {
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: any = {
        ...
        subcategory_id: pSubId ? Number(pSubId) : null,   // ── NEW ──
        price: Number(pPrice),
        stock: Number(pStock), unit: pUnit.trim(),
        image_url: imageUrl, ingredients: pIngredients.trim(), is_active: pAvailable,
      };
      ...
      setProductModal(false); setPFile(null);
      fetchProducts(activeCatFilter, activeSubFilter);
    } catch (err: any) {
      showToast(err?.response?.data?.error || 'Failed to save product', 'error');
    } finally { setSaving(false); setUploading(false); }
  };
  const handleDeleteProduct = (id: string) =>
    askConfirm('Delete this product? This cannot be undone.', async () => {
      try {
        await menuManagementService.deleteProduct(id);
        showToast('Product deleted'); fetchProducts(activeCatFilter, activeSubFilter);
      } catch (err: any) { showToast(err?.response?.data?.message || err?.response?.data?.error || 'Delete failed', 'error'); }
    });
  const handleToggleProduct = async (prod: Product) => {
    try {
      ...
      <div className="mm-content">
        ...
        {activeTab === 'products' && (
          <div className="mm-tab-pane">
            {/* Category chips */}
            <div className="mm-chips">
              <div className="mm-chips-label"><Filter size={12} /> Category:</div>
              <button
                className={`mm-chip ${activeCatFilter === 'all' ? 'mm-chip-active' : ''}`}
            ...
            {/* ── NEW: Subcategory chips — shown once a category with subcategories is selected ── */}
            {activeCatFilter !== 'all' && visibleSubCats.length > 0 && (
              <div className="mm-chips mm-chips-sub">
                <div className="mm-chips-label"><GitBranch size={12} /> Subcategory:</div>
                <button
                  className={`mm-chip ${activeSubFilter === 'all' ? 'mm-chip-active' : ''}`}
                  onClick={() => handleSubFilter('all')}
                >All</button>
                {visibleSubCats.map(sc => (
                  <button
                    key={sc.id}
                    className={`mm-chip ${activeSubFilter === sc.id ? 'mm-chip-active' : ''}`}
                    onClick={() => handleSubFilter(sc.id)}
                  >{sc.name}</button>
                ))}
              </div>
            )}
            ...
            {loading ? (
              ...
              <EmptyState icon={<Utensils size={36} />} title="No products found" sub={searchTerm ? 'Try a different search term' : (activeSubFilter !== 'all' ? 'No products are assigned to this subcategory yet. Edit a product and choose this subcategory.' : 'Add your first product to get started')} action={!searchTerm ? { label: 'Add Product', onClick: openAddProduct } : undefined} />
            ) : (
              <div className="mm-table-wrap">
                <table className="mm-table">
                  <thead><tr>
                    <th>Product</th><th>Category</th><th>Subcategory</th><th>Price</th>
                    <th>Stock</th><th>Status</th><th className="mm-th-right">Actions</th>
                  </tr></thead>
                  <tbody>
                    {filteredProducts.map(row => (
                      <tr key={row.id}>
                        ...
                        <td><span className="mm-cat-pill">{row.categoryName || '—'}</span></td>
                        <td><span className="mm-cat-pill">{row.subcategoryName || '—'}</span></td>
                        <td>
                          <div className="mm-price-cell">
                            <span className="mm-price-main">KWD{row.price}</span>
                            {row.originalPrice && row.originalPrice > row.price && (
                  ...
                  <tbody>
                    {filteredSubCats.map(row => (
                      <tr key={row.id}>
                        <td><div className="mm-sm-img-wrap">{row.image ? <img src={row.image} alt={row.name} className="mm-sm-img" /> : <div className="mm-sm-img-placeholder"><GitBranch size={15} /></div>}</div></td>
                        <td><strong className="mm-cell-name">{row.name}</strong></td>
                        <td><span className="mm-cat-pill">{row.parentCategoryName || categories.find(c => c.id === row.parentCategoryId)?.name || row.parentCategoryId}</span></td>
                        <td><span className="mm-cell-muted">{row.description || '—'}</span></td>
                        <td><StatusBadge active={row.isActive} /></td>
                        <td className="mm-td-right">
                          <div className="mm-row-actions">
      ...
      <Modal isOpen={productModal} onClose={() => setProductModal(false)} icon={<Package size={16} />} title={selProduct ? 'Edit Product' : 'Add Product'} size="lg">
        <form onSubmit={handleSaveProduct} className="mm-form">
          ...
          <Row2>
            ...
            <Field label="Category" required>
              <div className="mm-select-wrap">
                {/* CHANGED: changing category clears the chosen subcategory */}
                <select className="mm-select mm-input" value={pCatId} onChange={e => { setPCatId(e.target.value); setPSubId(''); }}>
                  <option value="">Select…</option>
                  {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
                <ChevronDown size={13} className="mm-select-arrow" />
          ...
          {/* NEW: Subcategory (only lists subcategories of the selected category) */}
          <Field label="Subcategory">
            <div className="mm-select-wrap">
              <select className="mm-select mm-input" value={pSubId} onChange={e => setPSubId(e.target.value)} disabled={!pCatId}>
                <option value="">{pCatId ? 'None' : 'Select a category first'}</option>
                {subCats.filter(sc => sc.parentCategoryId === pCatId).map(sc => (
