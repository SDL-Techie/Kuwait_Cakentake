...
const DeliveryDashboard: React.FC = () => {
  ...
  // Only the first 5 orders of the active tab are shown in the queue list.
  const activeList = lists[activeTab];
  const visibleList = activeList.slice(0, 5);
  const activeTabMeta = TABS.find(t => t.key === activeTab)!;

  /* ── Recent activity (latest across delivered + proof pending, first 5) ── */
  const recentActivity = useMemo(() => {
    const combined = [...lists.proof, ...lists.delivered];
    return combined.slice(0, 5);
  }, [lists.proof, lists.delivered]);
