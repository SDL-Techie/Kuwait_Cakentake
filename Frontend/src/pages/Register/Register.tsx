...
export default function Register(): JSX.Element {
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    if (!isCompleteLocalPhone(phoneCountry, form.phone_no)) {
    ...
    try {
      const res = await storefrontApi.register( {
        ...
        last_name: form.lastName,
        email: form.email,
        phone_no: buildInternationalPhone(phoneCountry, form.phone_no),
        password: form.password,
      });
        ...
        <div className="form-group">
          ...
          <PhoneInput
            ...
            onCountryChange={(country) => {
              ...
              setForm((prev) => ({ ...prev, phone_no: "" }));
            }}
            onChange={(value) => setForm((prev) => ({ ...prev, phone_no: value }))}
            required
          />
        </div>
