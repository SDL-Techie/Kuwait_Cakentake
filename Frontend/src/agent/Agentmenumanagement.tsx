...
// //     placeholder="e.g. 1kg, Half kg, Small"
// //     disabled={busy}
// //   />
// // </label>
...
// //             <div className="am-profile-item">
// //     <span>Variant</span>
// //     <strong>{product.variant?.trim() ? product.variant : "—"}</strong>
// //   </div>
// //   <div className="am-profile-item">
// //     <span>Flavour</span>
// //     <strong>{product.flavour?.trim() ? product.flavour : "—"}</strong>
// //   </div>
// //           <div className="am-profile-item">
// //             <span>Created</span>
...
// //             {product.variant?.trim() ? product.variant : "—"}
// //           </td>
// //           <td data-label="Flavour">
// //             {product.flavour?.trim() ? product.flavour : "—"}
// //           </td>
...
// // ─────────────────────────────────────────────────────────────────────────────
// // Chip-list editor for variants / flavours (plain strings, no price modifier)
// // ─────────────────────────────────────────────────────────────────────────────
...
//       <span>{label}</span>
//       {values.length > 0 && (
//         <div className="am-option-chip-list">
//           {values.map((v) => (
//             <span key={v} className="am-option-chip">
//               {v}
//               <button
//                 type="button"
...
//         </div>
//       )}
//       <div className="am-option-add-row">
//         <input
//           type="text"
//           value={draft}
...
//           }}
//         />
//         <button type="button" className="am-btn am-btn--ghost" onClick={addValue} disabled={disabled}>
//           Add
//         </button>
//       </div>
//     </label>
...
//   const handleChange = (key: keyof Pick<ProductFormState, "agent_id" | "name" | "description" | "price" | "image" | "is_active">, value: string | boolean) => {
//     setForm((f) => ({ ...f, [key]: value as never }));
//   };
...
//             label="Flavours"
//             placeholder="e.g. Chocolate, Vanilla"
//             values={form.flavours}
//             onChange={(next) => setForm((f) => ({ ...f, flavours: next }))}
//             disabled={busy}
//           />
...
//           <div className="am-profile-item">
//             <span>Variants</span>
//             <strong>{product.variants?.length ? product.variants.join(", ") : "—"}</strong>
//           </div>
//           <div className="am-profile-item">
//             <span>Flavours</span>
//             <strong>{product.flavours?.length ? product.flavours.join(", ") : "—"}</strong>
//           </div>
//           <div className="am-profile-item">
//             <span>Created</span>
//             <strong>{formatDate(product.created_at)}</strong>
...
//                       </td>
//                       <td data-label="Flavour">
//                         {product.flavours?.length ? product.flavours.join(", ") : "—"}
//                       </td>
//                       <td data-label="Agent">{agentFullName(agent)}</td>
//                       <td data-label="Price">{formatCurrency(Number(product.price ?? 0))}</td>
//                       <td data-label="Status">
...
/** "1kg (+2.50)" / "1kg" when there's no price modifier. */
function formatOption(opt: AgentProductOption) {
  if (!opt.price_modifier) return opt.name;
  const sign = opt.price_modifier > 0 ? "+" : "";
  return `${opt.name} (${sign}${formatCurrency(opt.price_modifier)})`;
}

function formatOptionList(options: AgentProductOption[] | undefined | null) {
  if (!options || options.length === 0) return "—";
  return options.map(formatOption).join(", ");
}
...
// ─────────────────────────────────────────────────────────────────────────────
// Option-list editor for variants / flavours — each option is a name PLUS
// its own price add-on (e.g. "1kg" +0.00, "2kg" +3.50, "Chocolate" +1.00).
// ─────────────────────────────────────────────────────────────────────────────
...
function OptionListEditor({
  ...
  values: AgentProductOption[];
  onChange: (next: AgentProductOption[]) => void;
  disabled?: boolean;
}) {
  ...
  const addValue = () => {
    ...
    const price_modifier = draftPrice.trim() === "" ? 0 : Number(draftPrice);
    onChange([
      ...values,
      { name, price_modifier: Number.isFinite(price_modifier) ? price_modifier : 0 },
    ]);
    setDraftName("");
    setDraftPrice("");
  };
  ...
  const updatePrice = (name: string, priceStr: string) => {
    const price_modifier = priceStr.trim() === "" ? 0 : Number(priceStr);
    onChange(
      values.map((v) =>
        v.name === name
          ? { ...v, price_modifier: Number.isFinite(price_modifier) ? price_modifier : 0 }
          : v
      )
    );
  };
  ...
  return (
    <label className="am-field am-field--full">
      ...
      {values.length > 0 && (
        <div className="am-option-chip-list">
          {values.map((v) => (
            <span key={v.name} className="am-option-chip">
              {v.name}
              <input
                type="number"
                step="0.01"
                className="am-option-chip-price"
                value={v.price_modifier}
                onChange={(e) => updatePrice(v.name, e.target.value)}
                disabled={disabled}
                aria-label={`Price add-on for ${v.name}`}
                style={{ width: 64, marginLeft: 6 }}
              />
              <button
                type="button"
                onClick={() => removeValue(v.name)}
                disabled={disabled}
    ...
    >
      <form className="am-form" onSubmit={handleSubmit} noValidate>
        <div className="am-form-grid">
          ...
          <OptionListEditor
            label="Variants (each can add to the base price)"
            namePlaceholder="e.g. 1kg, Half kg, Small"
            values={form.variants}
            onChange={(next) => setForm((f) => ({ ...f, variants: next }))}
            disabled={busy}
          />
          ...
          <OptionListEditor
            label="Flavours (each can add to the base price)"
            namePlaceholder="e.g. Chocolate, Vanilla"
            values={form.flavours}
            onChange={(next) => setForm((f) => ({ ...f, flavours: next }))}
            disabled={busy}
          />
    ...
    <ModalShell title="Product Details" onClose={onClose} width="md">
      <div className="am-profile">
        ...
        <div className="am-profile-grid">
          ...
          <div className="am-profile-item">
            <span>Variants</span>
            <strong>{formatOptionList(product.variants)}</strong>
          </div>
          <div className="am-profile-item">
            <span>Flavours</span>
            <strong>{formatOptionList(product.flavours)}</strong>
          </div>
          <div className="am-profile-item">
            <span>Created</span>
            <strong>{formatDate(product.created_at)}</strong>
          ...
          <table className="am-table">
            <thead>
              <tr>
                ...
                <th className="am-th-sortable" onClick={() => toggleSort("price")}>
                  Base Price {sortIndicator("price")}
                </th>
                <th className="am-th-sortable" onClick={() => toggleSort("is_active")}>
                  Status {sortIndicator("is_active")}
                </th>
                ...
                pageItems.map((product) => {
                  ...
                  return (
                    <tr key={product.id}>
                      ...
                      <td data-label="Variant">{formatOptionList(product.variants)}</td>
                      <td data-label="Flavour">{formatOptionList(product.flavours)}</td>
                      <td data-label="Agent">{agentFullName(agent)}</td>
                      <td data-label="Base Price">{formatCurrency(Number(product.price ?? 0))}</td>
                      <td data-label="Status">
                        <StatusBadge active={product.is_active} />
                      </td>
                      <td data-label="Created Date">{formatDate(product.created_at)}</td>
