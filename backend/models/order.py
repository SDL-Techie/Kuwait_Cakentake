...
#     agent_settlement_status = db.Column(db.String(20), nullable=False, default="PENDING")
#     agent_settled_at = db.Column(db.DateTime, nullable=True)
#     agent_settled_by = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=True)
...
#     delivery_confirmer = db.relationship(
#         "User",
...
#             "delivery_confirmed_user": self._user_brief(self.delivery_confirmer),
#             "pickup_handed_by": (
#                 self._user_brief(self.delivery_confirmer)
#                 if str(self.delivery_method or "DELIVERY").upper() == "PICKUP"
#                 and self.delivery_confirmed_at
#                 else None
#             ),
#             "pickup_handed_at": (
...
#             "agent_settlement_status": self.agent_settlement_status or "PENDING",
#             "agent_settled_at": self.agent_settled_at.isoformat() if self.agent_settled_at else None,
#             "agent_settled_by": self.agent_settled_by,
...
class Order(db.Model):
    ...
    agent_settlement_status = db.Column(db.String(20), nullable=False, default="PENDING")
    agent_settled_at = db.Column(db.DateTime(timezone=True), nullable=True)
    agent_settled_by = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=True)
...
# TAP
    ...
    def to_dict(self):
        return {
            ...
            "kitchen_staff_id": self.kitchen_staff_id,
            "kitchen_staff": self._user_brief(self.kitchen_staff),
            # FIX: use timezone-safe helper instead of raw .isoformat()
            "kitchen_assigned_at": _iso_utc(self.kitchen_assigned_at),
            "preparation_started_at": _iso_utc(self.preparation_started_at),
            "preparation_started_by": self._user_brief(
            self.preparation_started_user
            ),

            "completed_by_kitchen_at": _iso_utc(self.completed_by_kitchen_at),

            # Delivery agent (dispatcher) — distinct from driver
            "delivery_agent_id": self.delivery_agent_id,
            "delivery_agent": self._user_brief(self.delivery_agent),
            "delivery_agent_assigned_at": _iso_utc(self.delivery_agent_assigned_at),

            # Driver — distinct from delivery agent
            "driver_id": self.driver_id,
            "driver": self._user_brief(self.driver),
            "driver_assigned_at": _iso_utc(self.driver_assigned_at),
            "driver_accepted_at": _iso_utc(self.driver_accepted_at),
            "driver_submitted_at": _iso_utc(self.driver_submitted_at),

            "out_for_delivery_at": _iso_utc(self.out_for_delivery_at),
            "order_date": _iso_utc(self.order_date),
            ...
            "pickup_handed_at": (
                _iso_utc(self.delivery_confirmed_at)
                if str(self.delivery_method or "DELIVERY").upper() == "PICKUP"
                and self.delivery_confirmed_at
                else None
            ),
            "delivery_confirmed_at": _iso_utc(self.delivery_confirmed_at),
            ...
            # FIX: this used to be _iso_utc(self.created_at) — a copy/paste bug
            # that made updated_at always equal created_at.
            ...
            "driver_settlement_id": self.driver_settlement_id,
            "agent_settlement_status": self.agent_settlement_status or "PENDING",
            "agent_settled_at": _iso_utc(self.agent_settled_at),
            "agent_settled_by": self.agent_settled_by,
