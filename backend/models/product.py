...
# )
        ...
        if active_promotion and discounted_price is not None:
            discount_type = str(active_promotion.discount_type or "").upper()
            promo_type = str(active_promotion.promotion_type or "").upper()
            discount_value = float(active_promotion.discount_value or 0)

            if promo_type in {"DISCOUNT", "FLAT_OFFER"}:
                if discount_type in {"PERCENT", "PERCENTAGE"}:
                    discounted_price = max(
                        discounted_price - (discounted_price * discount_value / 100),
                        0,
                    )
                elif discount_type in {"FLAT", "FIXED"} or promo_type == "FLAT_OFFER":
                    discounted_price = max(discounted_price - discount_value, 0)

        decimals = 3 if str(currency).upper() == "KWD" else 2
        discounted_price = round(discounted_price, decimals) if discounted_price is not None else None 
