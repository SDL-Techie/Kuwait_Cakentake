...
// const IconPower = ({ size = 14 }: { size?: number }) => (
//   <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor"
//     strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const CustomerManagement: React.FC = () => {
  ...
  // ── Search / Sort / Status Filter / Pagination ──────────────────────────────
  const [searchQuery, setSearchQuery] = useState("");
  const [sortBy, setSortBy] = useState<SortOption>("newest");
  ...
  const handleToggleCustomerStatus = async (customer: Customer) => {
    const nextActive = !(customer.is_active ?? true);
    const confirmMsg = nextActive
      ? `Activate ${customer.first_name} ${customer.last_name}? They'll be able to log in and order again.`
      : `Deactivate ${customer.first_name} ${customer.last_name}? They won't be able to log in or place orders until reactivated.`;

    if (!window.confirm(confirmMsg)) return;

    setStatusActionLoading(customer.id);
    try {
      await updateCustomerStatus(customer.id, nextActive);
      showToast(nextActive ? "Customer activated" : "Customer deactivated", "success");

      setCustomers((prev) => {
        const updated = prev.map((c) =>
          c.id === customer.id ? { ...c, is_active: nextActive } : c
        );
        // If the current status filter no longer matches this customer, drop it from view
        if (
          (statusFilter === "active" && !nextActive) ||
          (statusFilter === "inactive" && nextActive)
        ) {
          return updated.filter((c) => c.id !== customer.id);
        }
        return updated;
      });

      // Keep the drawer profile in sync if it's open for this customer
      setDrawerCustomer((prev) =>
        prev && prev.id === customer.id ? { ...prev, is_active: nextActive } : prev
      );
    } catch (err: any) {
      showToast(err?.response?.data?.error || "Failed to update customer status.", "error");
    } finally {
    ...
    <div className="cm-workspace-container">
      ...
      <div className="cm-stats-grid">
        ...
        <div className="cm-stat-card">
          ...
          <div className="cm-stat-body">
            <h3>{(stats.avgSpend)}</h3>
            <span>Average Spend</span>
          </div>
        </div>
      </div>

      {/* ── 2. Search & Sort & Status Filter Bar ────────────────────────── */}
      <div className="cm-toolbar">
        <div className="cm-search-wrap">
          <IconSearch size={16} />
          <input
        ...
        <div className="cm-sort-wrap">
          <label className="cm-sort-label">Status</label>
          <select
            className="cm-sort-select"
            value={statusFilter}
                    ...
                    <tr>
                      <td className="empty-table-cell" colSpan={8}>
                        {searchQuery
                          ...
                          : statusFilter !== "all"
                          ? `No ${statusFilter === "active" ? "active" : "deactivated"} customers found.`
                    ...
                    paginatedCustomers.map((customer) => {
                      ...
                      return (
                        <tr key={customer.id}>
                          ...
                          <td>
                            <span className={`cm-status-pill ${isActive ? "cm-status-active" : "cm-status-inactive"}`}>
                          ...
                          <td>
                            <div className="tbl-actions">
                              ...
                              <button
                                className={`sage-btn btn-sm sm-icon-btn ${isActive ? "cm-btn-deactivate" : "cm-btn-activate"}`}
                ...
                <div className="cm-drawer-header">
                  <div className="cm-drawer-profile">
                    ...
                    <div className="cm-drawer-profile-info">
                      ...
                      <p>{drawerCustomer.email}</p>
                      <span className="sm-id-badge">#{drawerCustomer.id}</span>
                      <span className={`cm-status-pill ${drawerCustomer.is_active !== false ? "cm-status-active" : "cm-status-inactive"}`}>
                        {drawerCustomer.is_active !== false ? "Active" : "Deactivated"}
                      </span>
                    </div>
                  </div>
                  <button className="sage-modal-close" onClick={closeDrawer}>
