...
interface ProductData {
  ...
  images: string[];
  stock: number;
  unit: string; // ── FIX: was fetched from the backend but never stored/typed ──
  variants: Variant[];
  promotion?: PromotionData | null;
}
...
const ProductDetails: React.FC = () => {
  ...
  // ── FIX: currency now lives in state instead of being read ad-hoc from
  // localStorage inside the effect. This lets us (a) re-fetch the product
  ...
  useEffect(() => {
    const fetchProductAndWishlistStatus = async () => {
      ...
      try {
        // 1. Fetch data using your clean asynchronous service layout.
        // ── FIX: pass the resolved `currency` through explicitly instead of
        // letting getProductById guess it internally. This is the same
        // pattern already used below for getWishlist(userId, currency).
        // If getProductById's signature doesn't currently accept a second
        // arg, add one there that sets the 'X-Currency' header — the Flask
        // route already reads that header (see product.py get_product). ──
        const data = await getProductById(Number(id), currency);
        ...
        setProduct({
          ...
          categoryName: data.category?.name || "Cake",
          stock: data.stock,
          unit: data.unit || "", // ── FIX: now actually stored ──
          variants: activeVariants,
          promotion: data.promotion || null,
        });
    ...
    fetchProductAndWishlistStatus();
    // ── FIX: `currency` added so a currency change refetches the product
    // with the correct price instead of leaving the stale value on screen. ──
  }, [id, navigate, userId, currency]);
