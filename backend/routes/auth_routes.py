...
def login():
    ...
    user = User.query.filter(
        (db.func.lower(User.email) == email_or_phone.lower()) |
        (User.phone_no == email_or_phone)
    ).first()

    env_email = str(
        os.getenv("DEFAULT_ADMIN_EMAIL") or ""
    ).strip().lower()
    ...
    if (
        email_or_phone.lower() == env_email
        and password == env_password
        and env_email
        and env_password
    ):
        if not user:
    ...
    if not bool(getattr(user, "is_active", True)):
        return jsonify({
            "error": "Your account was deactivated. Please contact CakeNTake.",
