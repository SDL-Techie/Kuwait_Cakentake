...
def _calculate_product_promotion_savings(items_data):
    """Return the product-promotion savings already included in the checkout line prices."""
    savings = Decimal("0")
    ...
    for item in items_data or []:
        ...
        if original_price is None:
            original_price = custom.get("original_price")

        discounted_price = item.get("discounted_price")
        if discounted_price is None:
            discounted_price = item.get("price")
        if discounted_price is None:
            discounted_price = custom.get("unit_price")

        if original_price is None or discounted_price is None:
            continue

        try:
            original_dec = Decimal(str(original_price))
            discounted_dec = Decimal(str(discounted_price))
        except (InvalidOperation, TypeError, ValueError):
            continue

        if original_dec < discounted_dec:
            continue
        ...
        savings += (original_dec - discounted_dec) * Decimal(qty)
...
def create_order():
    ...
    promotion_savings = _calculate_product_promotion_savings(items_data)
    discount = max(Decimal("0"), discount - promotion_savings)
