...
from models.inventory import Inventory, InventoryConsumption
...
def complete_kitchen(order_id):
    ...
    # Record and deduct materials used for this order.
    data = request.get_json(silent=True) or {}
    consumptions = data.get("inventory_consumptions") or data.get("materials_used") or []
    for row in consumptions:
        material_id = row.get("material_id")
        quantity_used = float(row.get("quantity_used") or row.get("quantity") or 0)
        if not material_id or quantity_used <= 0:
            continue
        inventory = Inventory.query.filter_by(material_id=material_id).with_for_update().first()
        if not inventory:
            return jsonify({"error": f"Inventory not found for material {material_id}"}), 400
        if float(inventory.quantity or 0) < quantity_used:
            return jsonify({"error": f"Insufficient inventory for material {material_id}"}), 400
        inventory.quantity = float(inventory.quantity or 0) - quantity_used
        db.session.add(InventoryConsumption(
            material_id=material_id, order_id=order.id, quantity_used=quantity_used,
            notes=row.get("notes") or f"Used for order {order.order_number}",
        ))

    # stamp completion
    order.completed_by_kitchen_at = datetime.utcnow()

    # Pickup orders go back to Owner / Shop Manager after kitchen completion.
    if str(getattr(order, "delivery_method", "DELIVERY") or "DELIVERY").upper() == "PICKUP":
        order.status = "PICKUP_READY"
        log_order_status(order, old_status, "PICKUP_READY", current_user.id, "Kitchen completed pickup order; waiting for customer collection")
        db.session.commit()
        return jsonify({"message": "Pickup order is ready for customer collection", "order": order.to_dict()}), 200
    ...
    try:
        send_transactional_push(
            ...
            {"action": "OPEN_ORDER", "type": "DELIVERY_AGENT_ORDER_ASSIGNED", "target_id": order.id, "order_id": order.id},
        )
    except Exception as push_error:
        print("Kitchen completion push error:", push_error)
