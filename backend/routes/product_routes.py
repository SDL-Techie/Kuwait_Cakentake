...
def create_product():
    try:
        ...
        try:
            user_ids = [row.id for row in User.query.filter_by(role="USER", is_active=True).with_entities(User.id).all()]
            send_transactional_push(
                user_ids,
                "New cake available",
                f"{product.name} is now available on Cake N Take.",
                {"action": "OPEN_PRODUCT", "type": "NEW_PRODUCT", "target_id": product.id, "product_id": product.id},
            )
        except Exception as push_error:
            print("Product push notification error:", push_error)
