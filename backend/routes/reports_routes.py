...
def report_sales():
    ...
    total_sales = sum(float(o.grand_total or o.total or 0) for o in orders)
    return jsonify({"total_orders": len(orders), "total_sales": total_sales, "sales_total": total_sales, "total_revenue": total_sales, "average_order_value": (total_sales / len(orders)) if orders else 0}), 200
...
def report_revenue():
    ...
    series = [{"date": str(r[0]), "amount": float(r[1] or 0)} for r in results]
    total_revenue = sum(x["amount"] for x in series)
    return jsonify({"revenue": series, "total_revenue": total_revenue, "revenue_total": total_revenue}), 200
