...
def get_customers():
    ...
    # Aggregate order stats for every customer in ONE query instead of N+1
    ...
    order_stats = {
        row[0]: {"total_orders": row[1], "total_spent": float(row[2] or 0)}
        for row in order_stats_rows
    }

    result = []
    for c in customers:
        data = c.to_dict()  # already includes is_active
        stats = order_stats.get(c.id, {"total_orders": 0, "total_spent": 0.0})
...
@customers_bp.route("/customers/<int:customer_id>/status", methods=["PATCH"])
@jwt_required()
...
def update_customer_status(customer_id):
    ...
    return jsonify({
        "message": "Customer activated" if customer.is_active else "Customer deactivated",
...
def get_customer(customer_id):
    ...
    stats = db.session.query(
        func.count(Order.id).label("orders"),
        func.sum(Order.total).label("total_spent"),
    ).filter(Order.user_id == customer_id).first()

    data = customer.to_dict()
    data["total_orders"] = stats.orders or 0
    data["total_spent"] = float(stats.total_spent or 0)
