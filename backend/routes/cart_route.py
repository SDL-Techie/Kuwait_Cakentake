...
#     if flavor_id:
#         flavor = Flavor.query.get(flavor_id)
#         if not flavor or (variant and flavor.variant_id != variant.id):
#             return jsonify({"error": "Invalid flavor for this variant"}), 400
#         unit_price += float(flavor.price_modifier or 0)
...
def add_to_cart():
    ...
    # Use the product's own price as-is — it already reflects any active
    # promotion discount (see Product.to_dict()). No variant/flavor/addon
    # modifiers are applied here; the cart always shows the same price the
    # customer saw on the product page.
    unit_price = float(product.price)
    ...
    # Merge purely by product — no customizations affect the cart anymore.
    ...
    if cart_item:
        cart_item.quantity += quantity
        cart_item.total_price = unit_price  # refresh in case price changed
    else:
        cart_item = CartItem(
            cart_id=cart.id,
            product_id=product_id,
